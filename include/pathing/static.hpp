...
class RRT {
 public:
  ...
  std::vector<std::pair<RRTNode *, RRTOption>> pathingOptions(
      const RRTPoint &end, int quantity_options = 10) {
    std::vector<std::pair<RRTNode *, RRTOption>> options;
   ...
   */
  void fillOptions(std::vector<std::pair<RRTNode *, RRTOption>> &options,
                   RRTNode *node) {
    if (node == nullptr) {
      return;
    }
    ...
    std::vector<RRTOption> local_options =
        _dubins.allOptions(node->getPoint(), _goal);
   ...
   * @param sample    ==> sampled point
   */
  void optimizeTreeRecursive(RRTNode *node, RRTNode *prev_node,
                             RRTNode &sample) {
    if (node == nullptr) {
      return;
    }
    ...
    for (const RRTOption &option : options) {
      ...
      std::vector<XYZCoord> current_path =
          _dubins.generatePoints(sample.getPoint(), node->getPoint(),
      ...
      if (node->getCost() + option.length >=
          _tree.getNode(sample.getPoint())->getCost()) {
        continue;
      }
