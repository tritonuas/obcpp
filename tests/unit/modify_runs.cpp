...
TEST(ModifyRuns, OverWriteData) {
    ...
    for (auto value : updated) {
        ...
        for (int i = 0; i < value.coordinates_size(); i++) {
            ASSERT_EQ(value.coordinates().at(i).altitude(),
                      ptr->at(value.run_id()).coordinates().at(i).altitude());
            ASSERT_EQ(value.coordinates().at(i).longitude(),
                      ptr->at(value.run_id()).coordinates().at(i).longitude());
            ASSERT_EQ(value.coordinates().at(i).latitude(),
                      ptr->at(value.run_id()).coordinates().at(i).latitude());
        }
    }
}