...
raspCam::raspCam(CameraConfig config) :
    CameraInterface(config) {
    this->connect();
    this->startStreaming();
}

void raspCam::connect() {
    if (this->isConnected()) {
        return;
    }
...
void raspCam::startTakingPictures(const std::chrono::milliseconds& interval,
    ...
    this->captureThread = std::thread(&raspCam::captureEvery, this, interval);
}

void raspCam::stopTakingPictures() {
    if (!this->isTakingPictures) {
        return;
    }
...
void raspCam::configureSettings() {
...
std::optional<ImageData> raspCam::getLatestImage() {
...
std::deque<ImageData> raspCam::getAllImages() {
    ReadLock lock(this->imageQueueLock);
...
void raspCam::captureEvery(const std::chrono::milliseconds& interval) {
    loguru::set_thread_name("raspiberry pi camera");
    if (!this->isConnected()) {
        LOG_F(ERROR, "RPI Camera not connected. Cannot capture photos");
        return;
    }
...
std::optional<ImageData> raspCam::takePicture(const std::chrono::milliseconds& timeout,
    std::shared_ptr<MavlinkClient> mavlinkClient) {
    if (!this->isConnected()) {
...
std::optional<cv::Mat> raspCam::imgConvert(Arena::IImage* pImage) {
