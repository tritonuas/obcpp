...
Eigen::Vector2d halfDisplacement(
    const Eigen::Vector2d &vector1,
    const Eigen::Vector2d &vector2) {
...
Eigen::Vector2d Dubins::findCenter(
    const RRTPoint &point,
    char side) const {
...
Eigen::Vector2d Dubins::circleArc(
    const RRTPoint &starting_point,
    double beta,
    const Eigen::Vector2d &center,
    double path_length) const {
    // Code is not nessisarily intuitive (I don't want to call sign twice)
    // starting_angle_(+- half_pi depending on the sign) +
    //      the_angular_distance_tranveled_(depending on the sign)
    double angle = starting_point.psi+ ((path_length / _radius) - HALF_PI) *
        sign(beta);
    Eigen::Vector2d direction_vector{std::cos(angle), std::sin(angle)};
    return center + (direction_vector * _radius);
}

std::vector<Eigen::Vector2d> Dubins::generatePointsStraight(
    ...
    if (std::abs(path.beta_0) > 0) {
        ...
        double angle = start.psi+ (std::abs(path.beta_0) - HALF_PI) *
            sign(path.beta_0);
        initial_terminal_point = center_0 + _radius *
            Eigen::Vector2d{std::cos(angle), std::sin(angle)};
    } else {
        initial_terminal_point = Eigen::Vector2d{start.xyz.x, start.xyz.y};
    }

    if (std::abs(path.beta_2) > 0) {
        // negative sign before beta_2 is because the path comes in
        // through the back of the end vector
        double angle = end.psi+ (-std::abs(path.beta_2) - HALF_PI) *
            sign(path.beta_2);
        final_terminal_point = center_2 +
            _radius * Eigen::Vector2d{std::cos(angle), std::sin(angle)};
    } else {
        final_terminal_point = Eigen::Vector2d{end.xyz.x, end.xyz.y};
    }
    ...
    for (double current_distance = 0;
        current_distance < total_distance;
        current_distance += _point_separation) {
        if (current_distance < std::abs(path.beta_0) * _radius) {
            // First turn
            points_list.emplace_back(circleArc(
                start,
                path.beta_0,
                center_0,
                current_distance));
        } else if (current_distance >
            total_distance - std::abs(path.beta_2) * _radius) {
            // Last turn
            // need to calculate new "start" point,
            // which is the difference in end angle to turn angle
            RRTPoint final_point_RRT{XYZCoord{
                    final_terminal_point.x(),
                    final_terminal_point.y(),
                    0},
                end.psi- path.beta_2};
            ...
            // Straight Section
            // coefficient is the ratio of the straight distance
            ...
            // convex linear combination to find the vector along the
            // straight path between the initial and final point
    ...
    points_list.emplace_back(Eigen::Vector2d{end.xyz.x, end.xyz.y});

    return points_list;
}

std::vector<Eigen::Vector2d> Dubins::generatePointsCurve(
    const RRTPoint &start,
    const RRTPoint &end,
    const DubinsPath &path) const {
    ...
    Eigen::Vector2d center_1 = (center_0 + center_2) / 2  // midpoint
        ...
        // a - intercenter_distance / 2
        //      (half of distance between center 0 and center 2)
        * sqrt(std::pow(2 * _radius, 2) -
            std::pow(half_intercenter_distance, 2))
        // unit vector (direction vector) orthogonal to the
        //      displacement vector between the two centers
        * findOrthogonalVector2D(center_2 - center_0).normalized();

    // angle between x+ and the "terminal vector" between turn 1 and turn 2
    // i.e the angle from center_1 (x+) to center_0
    // angle of the displacement vector relative to x+ (horizontal) - 180 deg
    // [supplimentary interior angle]
    double psi_0 = std::atan2(
        center_1.y() - center_0.y(),
        center_1.x() - center_0.x()) - M_PI;
    ...
    for ( double current_distance = 0;
        ...
        if (current_distance < std::abs(path.beta_0) * _radius) {
            ...
            points_list.emplace_back(circleArc(
                ...
                center_0,
                current_distance));
        } else if (current_distance > total_distance - std::abs(path.beta_2)
                    * _radius) {
            // Last Turn
            points_list.emplace_back(circleArc(
                end,
            ...
            // starting angle - (1 if LRL, -1 if RLR, *
            //      (total angular distance - angular distance from first turn))
            // *note, it is subtracting because the sign of the
            //      middle curve is always opposite to the start curve
            double angle = psi_0 - (sign(path.beta_0) *
                (current_distance / _radius - std::abs(path.beta_0)));
            Eigen::Vector2d direction_vector{std::cos(angle), std::sin(angle)};
...
std::vector<Eigen::Vector2d> Dubins::generatePoints(
    const RRTPoint &start,
    const RRTPoint &end,
...
RRTOption Dubins::lsl(
    const RRTPoint &start,
    const RRTPoint &end,
    const Eigen::Vector2d &center_0,
    const Eigen::Vector2d &center_2) const {

    double straight_distance = distanceBetween(center_0, center_2);

    // angle relative to horizontal
    double alpha = std::atan2(
        center_2.y() - center_0.y(),
        center_2.x() - center_0.x());
...
std::vector<RRTOption> Dubins::allOptions(
    const RRTPoint &start,
    const RRTPoint &end,
    bool sort) const {
...
std::vector<Eigen::Vector2d> Dubins::dubinsPath(
    const RRTPoint &start,
